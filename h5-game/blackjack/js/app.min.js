...
function initDeal() {
	...
	aPCards2 = new Array;
	//===============第一次发牌==========
	if (interfaceData.result != 1 ) {
		//如果要玩家输,就不让玩家得到21点
		var cardsCache = getCards(2);
		if (roundToTen(cardsCache[0].value) + roundToTen(cardsCache[1].value) == 11 && (cardsCache[0].value == 1 || cardsCache[1].value == 1)) {
			cardsCache[1] = getRangeCards(cardsCache[1],cardsCache,21,21,false);//(cardsCache[1],cardsCache,1,10);
		}
		aPCards1 = aCurPlayerCards = cardsCache;
	}else {
		aPCards1 = aCurPlayerCards = getCards(2);
	}
	//===================================-
	aCompCards = getCards(1);
	//aCompCards[0] = reGetCards(aCompCards[0],1);//庄家的明牌拿一张A	
	//如果玩家拿到的前两张牌是一张A和一张10点牌，就拥有黑杰克（Black jack）
	if (roundToTen(this.aCurPlayerCards[0].value) + roundToTen(this.aCurPlayerCards[1].value) == 11 && (this.aCurPlayerCards[0].value == 1 || this.aCurPlayerCards[1].value == 1)) {
		...
		//分牌(SPLIT) 在21点里，遇到两张牌相同的点数，是可以分牌的
		dealType = "split";
		userInput.addHitArea("split", butEventHandler, null, {//分牌
			type: "rect",
			aRect: [0, 630, 133, 758]
		})
	...
	if (aCompCards[0].value == 1 && bank >= bet) {
		//INSURE【保险】
		//如果庄家的明牌有一张A，则玩家可以考虑买不买保险，金额是赌筹的一半且保险金不予退还。
		//如果庄家是blackjack，那么玩家拿回投注金额；如果庄家没有blackjack则玩家继续游戏
		userInput.addHitArea("insure", butEventHandler, null, {  //保险
			type: "rect",
			aRect: [0, 200, 150, 350]
		});
		e = true
	}
	if (bank >= bet && bet<=interfaceData.doubleMax) { //双倍 1000赌注封顶 赌注大于500就没有双倍.
	...
	//Deal开始。HIT抽牌，STAND停止抽牌，DOUBLE双倍赌注
...
function initCompPlay() {
	...
	var pre_point = getBestValue(aCompCards);  //庄家点数
	var play_point = getBestValue(aCurPlayerCards); //玩家点数
	aCompCards.push(e);
	//控制 庄家拿第二张牌======================
	//console.log('玩家点数:'+play_point);
	//console.log('庄家点数:'+ pre_point+'===>'+ getBestValue(aCompCards));
	if(interfaceData.result == 0 ) {
        if(play_point==17 && getBestValue(aCompCards)==17) {
			e = getRangeCards(e,aCompCards,17,17,false);//拿一张不为17的牌
		}
		if(play_point>17 && getBestValue(aCompCards)>=17 && getBestValue(aCompCards)<=play_point) {
			e = getRangeCards(e,aCompCards,17,play_point,false);//拿一张 牌
		}
	}
	if(interfaceData.result == 1 ) {
		//控制 玩家 赢  =========>
		if(play_point<=17 &&  getBestValue(aCompCards)>=17 ) {
			e = getRangeCards(e,aCompCards,17,50,false);//拿一张 的牌
		}
		if(play_point>17 &&  getBestValue(aCompCards)>=play_point ) {
			e = getRangeCards(e,aCompCards,play_point,50,false);//拿一张 牌
		}
	}
	if(interfaceData.result == 2 &&  play_point>=17 && getBestValue(aCompCards)!=play_point) {
		//控制 玩家 平===========>  getBestValue(aCompCards) <= getBestValue(aCurPlayerCards)
		e = getTieCards(e,aCompCards,play_point);//拿一张 牌
	}
	aCompCards.pop();
	aCompCards.push(e);
	//==================================
	//console.log(aCompCards);
	playSound("cardSoft");
	panel.setUp("compPlay", {
		value: getBestValue(aCompCards)
	});
...
function dealNewCompCard() {
	...
	//console.log('庄家点数:'+ pre_point+'===>'+ getBestValue(aCompCards));
	if(interfaceData.result == 0 ) {
		if(play_point<17 && getBestValue(aCompCards)>=22) {
			e = getRangeCards(e,aCompCards,22,50,true);//拿一张不bust的牌
		}
		if(play_point>=17 && ( (getBestValue(aCompCards)>=17  && getBestValue(aCompCards)<=play_point) || getBestValue(aCompCards)>21)) {
			e = getRangeCards(e,aCompCards,17,play_point,true);//拿一张 牌
		}
	}
	if(interfaceData.result == 1 ) {
		//控制 玩家 赢  =========>
		if(play_point<=17 && getBestValue(aCompCards)>=17  && getBestValue(aCompCards)<=21) {
			e = getRangeCards(e,aCompCards,17,21,false);//拿一张 的牌
		}
		if(play_point>17 && getBestValue(aCompCards)>=play_point  && getBestValue(aCompCards)<=21) {
			e = getRangeCards(e,aCompCards,play_point+1,21,false);//拿一张 牌
		}
	}
	if(interfaceData.result == 2 &&  play_point>=17 && getBestValue(aCompCards)!=play_point ) {
		//控制 玩家 平===========>  getBestValue(aCompCards) <= getBestValue(aCurPlayerCards)
...
function checkInsuranceHand() {
	...
	if (e == 21) {
		bank += Math.round(bet * 1.5);//保险赢 是 21
		gameEnd(21,'21点 保险赢 ',bank,0);//调用后台接口
		bet = 0;
...
function checkHands() {
	...
	if (e > 21) {
		...
		playSound("win");
		bank += bet*2;
		gameEnd(1,'  庄家暴 玩家赢',bank,bet);//调用后台接口
		bet = 0;
		userInput.removeHitArea("split");
		userInput.removeHitArea("double");
		userInput.removeHitArea("hit");
		...
		if (e > t) {
			...
			playSound("bust");
			gameEnd(2,' 庄家Win',bank,-bet);//调用后台接口
			//if (bank < 5) {
			//	bank = 1e3
			//}
			bet = 0;
			...
			playSound("win");
			bank += bet*2;
			gameEnd(3,'玩家大 赢 ',bank,bet);//调用后台接口
			bet = 0;
			userInput.removeHitArea("split");
			userInput.removeHitArea("double");
			userInput.removeHitArea("hit");
...
function checkBlackjack() {
	var e = getBestValue(aCompCards);
	if (e == 21) {
		//平手(PUSH)：即玩家手上牌的点数与庄家的点数相同，或同是黑杰克的时候，双方便没有输赢，投注金将退回给玩家。
		panel.setUp("push");
		playSound("push");
		bank += bet;
		gameEnd(21,'checkBlackjack==21 ??',bank,bet);//调用后台接口
		bet = 0;
		numbers.setAmounts(bet, bank);
		userInput.removeHitArea("split");
		userInput.removeHitArea("double");
		...
		playSound("blackjack");
		//赔率
		bank += bet + Math.round(bet*interfaceData.twentyOneOdds); //21赔率
		gameEnd(21,'21点=blackjack  玩家 赢',bank,Math.round(bet * interfaceData.twentyOneOdds));//调用后台接口
		bet = 0;
		numbers.setAmounts(bet, bank);
		userInput.removeHitArea("split");
		userInput.removeHitArea("double");
...
//从52张扑克牌中 重新 随机抽取 1张牌  old旧牌  取大于n小于m的牌 并且牌面大小与old旧牌不一样
function reGetCards(old,n) {
	aDeck.push(old);
	var t = new Array;	
	var r = 0;
	//console.log(n,m,r);
	if (aDeck.length > 0) {
		for( var times=0;times<200;times++ )	{
			r = Math.floor(Math.random() * aDeck.length);			
			//console.log('换牌:',aDeck[r].value);
			if(roundToTen(aDeck[r].value) == n  ){
				times=201;
			}
		};		
		t.push(aDeck[r]);
...
//从52张扑克牌中 随机抽取 排除指定范围内的 1张牌
function getRangeCards(old,precards,i,j,noBust) {
	aDeck.push(old);
	var t = new Array;
	var r = 0;
	//console.log(n,m,r);
	if (aDeck.length > 0) {
		for( var times=0;times<200;times++ )	{
			r = Math.floor(Math.random() * aDeck.length);
			precards[precards.length-1] = aDeck[r];
			//console.log('换牌:',aDeck[r].value);
			if( (getBestValue(precards) >= i && getBestValue(precards) <= j) || (roundToTen(aDeck[r].value) == roundToTen(old.value)) || (noBust && getBestValue(precards)>21)){
				//
			}else {
					times=201;
			}
...
//从52张扑克牌中 随机抽取1张牌 for平局
function getTieCards(old,precards,play_point) {
	aDeck.push(old);
	var t = new Array;
	var r = 0;
	//console.log(n,m,r);
	if (aDeck.length > 0) {
		for( var times=0;times<200;times++ )	{
			...
			precards[precards.length-1] = aDeck[r];
			//console.log('换牌:',aDeck[r].value);
			if( (getBestValue(precards) < 17 || getBestValue(precards) == play_point ) && roundToTen(aDeck[r].value) != roundToTen(old.value)){
					times=201;  //已取到要的牌
			}
		};
		t.push(aDeck[r]);
...
function dealNewPlayerCard(e) {
	...
	var t = getCards(1)[0];
	var play_cards = $.map( aCurPlayerCards, function(obj){
		return $.extend(true,{},obj);//返回对象的深拷贝
	});
	play_cards.push(t);
	//控制 玩家拿牌============================
	if(interfaceData.result == 0 && getBestValue(play_cards) == 21 && getBestValue(aCurPlayerCards)<21) {
		// 玩家输 ==========>  != 21
		t = play_cards[play_cards.length-1] = getRangeCards(t,play_cards,21,21,false);//(t,play_cards,0,22); //换张不一样的牌
	}
	if(interfaceData.result != 0 && getBestValue(play_cards) > 21 && getBestValue(aCurPlayerCards)<21) {
		// 玩家赢 或 平=====>  <= 21
		//console.log('控制前'+getBestValue(play_cards));
		t = play_cards[play_cards.length-1] =getRangeCards(t,play_cards,22,50,true); //(t,play_cards,0,22); //换张不会爆的牌
		//console.log('控制后'+getBestValue(play_cards));
	}
	...
	if (getBestValue(aCurPlayerCards) > 21) {
		...
		if (!splitTaken) {
			if (getBestValue(aPCards1) > 21) {
				...
				gameEnd(5,'玩家 暴 bust',bank,-bet);//调用后台接口
				//这里是处理 如果玩家没钱了 这里是送1000，如果要修改 就修改这里
				if (bank < 5) {
					//bank = 1e3
				}
				buttons.setUp("bust");
			...
			//如果庄家的明牌有一张A，则玩家可以考虑买不买保险，金额是赌筹的一半且保险金不予退还。如果庄家是blackjack，那么玩家拿回投注金额；如果庄家没有blackjack则玩家继续游戏
			bet /= 2;
			numbers.setAmounts(bet, bank);
	...
	switch (e) {
		...
		case "addChip":
			if (aChipAmounts[t.id] <= bank) {
				...
				playSound("chip");
				panel.showDealBut();
				if(bet+aChipAmounts[t.id + chipLevel]>interfaceData.betMax) return; //赌注封顶
		...
		case "insure":
			...
			if (roundToTen(insuranceCard.value) == 10 && interfaceData.result > 0 ) {
				//insuranceCard=getRangeCards(t,play_cards,21,21,false);
			}
			bank -= Math.round(bet / 2); //买保险.
			numbers.setAmounts(bet, bank);
			insuranceTaken = true;
...
//加载资源
function loadAssets() {
	//资源采取压缩图片的 格式 然后 通过读取大图 分割小图的方式组合
...
(function(e) {
	var t = function() {
		...
		e.prototype.updateChipStacks = function() {
			//更新筹码  5  10 25 100 这个4个筹码
			//chipLevel 这个是游戏等级
			if (this.bank < this.aChipAmounts[0 + this.chipLevel] || this.aChipAmounts[0 + this.chipLevel]+bet>interfaceData.betMax ) {//赌注封顶,移除可封顶的筹码.
				...
				if (this.bank < this.aChipAmounts[1 + this.chipLevel] || this.aChipAmounts[1 + this.chipLevel]+bet>interfaceData.betMax) {//赌注封顶,移除可封顶的筹码.
					this.betLimit();
					this.aChipStacks[1].y = 800;
					this.aChipStacks[2].y = 800;
					this.aChipStacks[3].y = 800;
					...
					if (this.bank < this.aChipAmounts[2 + this.chipLevel] || this.aChipAmounts[2 + this.chipLevel]+bet>interfaceData.betMax) {//赌注封顶,移除可封顶的筹码.
						this.aChipStacks[2].y = 800;
						this.aChipStacks[3].y = 800;
					} else {
						this.aChipStacks[2].y = this.aChipStacks[2].targY;
						if (this.bank < this.aChipAmounts[3 + this.chipLevel] || this.aChipAmounts[3 + this.chipLevel]+bet>interfaceData.betMax) {//赌注封顶,移除可封顶的筹码.
							this.aChipStacks[3].y = 800;return;
						} else {
...
var sound;//声音
var music;
var allowSound = false; //允许声音
var muted = false;//静音
var splash;//loading
var splashTimer = 0;//loading时间
var screens;
var assetLib;//资源管理器
var preAssetLib;
...
var isMobile = false;
var gameState = "loading";//状态机  开始加载
 ...
 *
 * */
var controlCount={   //测试统计数据
	rate:interfaceData.rate,       //玩家赢的概率
	tie_rate:interfaceData.tie_rate,    //平局的概率
	all : 0,
	win_ctrl : 0,
	lost_ctrl : 0,
	tie_ctrl : 0,
...
function test() {
	controlCount.win_ctrl_rate = (controlCount.win/controlCount.win_ctrl*100).toFixed(1);
	controlCount.lost_ctrl_rate = (controlCount.lost/controlCount.lost_ctrl*100).toFixed(1);
	controlCount.tie_ctrl_rate = (controlCount.tie/controlCount.tie_ctrl*100).toFixed(1);
	controlCount.total_rate = '不知道!';
	console.log('============================');
	console.log('||  --------本次游戏设置为: 玩家赢的概率=:'+controlCount.rate+' 输的概率='+(100-controlCount.rate-controlCount.tie_rate)+' 平的概率='+controlCount.tie_rate+'%');
	console.log('||  玩家预定的输赢局数为: 赢:'+controlCount.win_ctrl+', 输:'+controlCount.lost_ctrl+', 平:'+controlCount.tie_ctrl+'');
	console.log('||  玩家实际的输赢局数为: 赢:'+controlCount.win+', 输:'+controlCount.lost+', 平:'+controlCount.tie+'');
	console.log('||');
	console.log('||  输赢的控制准确率为: 赢:'+controlCount.win_ctrl_rate+'%, 输:'+controlCount.lost_ctrl_rate+'%, 平:'+controlCount.tie_ctrl_rate+'%');
	console.log('||  最终的控制率为:'+controlCount.rate+'%');
	console.log('||  有没有被坑的感觉?');
	console.log('||  你赢的概率为:'+(controlCount.win/(controlCount.win+controlCount.lost+controlCount.tie)*100).toFixed(1)+'%');
	console.log('============================');
}
function setAccountBank () {
	updateBank(true,0);
}
function gameEnd(_n,_mmessage,_bank,_num,split_bust) {
	var chang_num=_bank - interfaceData.startBank;
	if(split_bust) chang_num = -bet;
	updateBank(false,chang_num);
}
// 单机版更新bank,
function sign_updateBank (init,num) {
	if(init) {
		bank=10000;
		interfaceData.startBank=10000;
	}else {
		bank = bank + num;
		interfaceData.startBank = bank;
		numbers.setAmounts(bet,bank);
		//统计数据
		if(num<0) controlCount.lost++;
		if(num==0) controlCount.tie++;
		if(num>0) controlCount.win++;
		test();
	}
	if (bank < 5) {//
		bank=10000;
		alert('你没钱了,给你冲了1W,接着玩吧!');
	}
	getRandom();//根据概率,得到本局 输赢平
}
// 联机版更新bank,
function updateBank (init,num) {
	if(interfaceData.token=='') {
		bank=0;//alert('您还没有登录,请登录后再进入游戏!');
		//下面是单机版代码,
		//sign_updateBank(init,num);
		//return;
	}
	$.ajax({
		type: 'post',
		headers:{'Authorization':interfaceData.token},
		url: interfaceData.updateAcountApi,
		data:{'num':num},
		...
		success: function(data){
			if(init) {
				...
				bank=data.ticket;interfaceData.startBank=bank;
				numbers.setAmounts(bet,bank);
				//统计数据
				if(num<0) controlCount.lost++;
				if(num==0) controlCount.tie++;
			...
			interfaceData.rate=data.probability;  //
			getRandom();//根据概率,得到下一局的 输赢平

		},
		error: function(xhr, type){
			alert('您还没有登录,请登录后再进入游戏!');
		}
	});
}
//根据概率,得到本局 输赢平
function getRandom() {
	var random_rate = Math.floor(Math.random()*100);
	if(random_rate < interfaceData.rate) {
		interfaceData.result = 1; //赢
		//console.log('下一局玩家 赢');
		controlCount.win_ctrl++;//统计数据
	}else if(random_rate >= interfaceData.rate && random_rate <= (100-interfaceData.tie_rate)) {
		interfaceData.result = 0; //输
		//console.log('下一局玩家 输');
		controlCount.lost_ctrl++;//统计数据
	}else {
		interfaceData.result = 2; //平
		//console.log('下一局 平');
		controlCount.tie_ctrl++;//统计数据
	}
	//console.log('游戏初始余额:'+interfaceData.startBank);
}